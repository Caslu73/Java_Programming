...
public class ArrayListPractice {
    ...
    public static void main(String[] args) {
        ...
        System.out.println("---------------------------------------------------------------");
            //Job title who are Java developer
        for (Employee each : employees) {
        ...
        double max = employees.get(0).salary;//Just adding as a parameter
        double min = employees.get(0).salary;
        //Maximum Salary
        for (Employee each : employees) {
            if(each.salary > max){//if each salary is greater than the actual salary
                max = each.salary; // so then assign to the max variable
            }
        }
        ...
        for (Employee employee : employees) {
            if(employee.gender == 'M'){//if the employee is male
                maleEmployees.add(employee);//if is male add those employee inside the maleEmployees
            }else{
        ...
        System.out.println("Total number of female employees: "+ femaleEmployees.size()); // size to know How much female employees
